...
function _lookupCommentEtc(res, etc) {
  ...
  if (['artist', 'group', 'series', 'tag'].includes(key)) {
    ...
    pool.query(
        'SELECT * FROM comment_' + keyv[0] + '_junction WHERE Target=?',
        [value], function(error, results, fields) {
          if (error != null) {
            logger.error('read-eh-comment');
            logger.error(error);
