...
function _artists(res, query) {
  ...
  if ('Artists' in query) {
    query.Artists.forEach(
        e => sql_union.push(
            '(select Article, ? as Artist from eharticles_artists_junction where Artist=(' +
  ...
  if ('Groups' in query) {
    query.Groups.forEach(
        e => sql_union.push(
            '(select Article, ? as Artist from eharticles_groups_junction where Group=(' +
  ...
  if ('Series' in query) {
    query.Series.forEach(
        e => sql_union.push(
            '(select Article, ? as Artist from eharticles_series_junction where Series=(' +
  ...
  if ('Characters' in query) {
    query.Characters.forEach(
        e => sql_union.push(
            '(select Article, ? as Artist from eharticles_characters_junction where Character=(' +
  ...
  if ('Tags' in query) {
    query.Tags.forEach(
        e => sql_union.push(
            '(select Article, ? as Artist from eharticles_tags_junction where Tag=(' +
