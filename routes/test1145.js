...
function _lookupViewTimeAvg(res) {
  ...
  const qr = pool.query(
      ...
      '(SELECT ArticleId, avg(ViewSeconds) as AvgOfViewSeconds, count(UserAppId) as HowManyUserRead from viewtime group by ArticleId order by AvgOfViewSeconds desc) as B '+
