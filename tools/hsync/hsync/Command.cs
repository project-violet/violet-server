...
//#if false
            ...
            // Make DataBase
            {
                ...
                // Estimate DateTime
                var mindd = xxx.Min(x => DateTime.Parse(x.Published));
                ...
                {
                    xx1.Add(int.Parse(xxx[i].URL.Split('/')[4]));
                    yy1.Add((DateTime.Parse(xxx[i].Published) - mindd).TotalMinutes);
                    if (!dict.ContainsKey(xxx[i].URL.Split('/')[4]))
                        dict.Add(xxx[i].URL.Split('/')[4], i);
                }
                var model = new PolynomialRegressionModel(Vector.Create(yy1.ToArray()), Vector.Create(xx1.ToArray()), 100);
                model.Fit();

                {
                    HitomiData.Instance.metadata_collection.Clear();
                    HitomiData.Instance.Load();
                    //var index = HitomiIndex.MakeIndexF();
...

                    {
                        ...
                        db.InsertAll(HitomiData.Instance.metadata_collection.Select(md =>
                        {
                            ...
                            else if (dd.Published == null)
                                dd.Published = mindd.AddMinutes(model.Predict(md.ID));

                            return dd;
                        }));
                        db.Close();

                        //var dbi = new SQLiteConnection("hitomidata-index.db");
                        //dbi.CreateTable<HitomiColumnModel>();
                        //dbi.CreateTable<HitomiIndexArtistsColumnModel>();
                        //dbi.CreateTable<HitomiIndexGroupsColumnModel>();
                        //dbi.CreateTable<HitomiIndexSeriesColumnModel>();
                        //dbi.CreateTable<HitomiIndexCharactersColumnModel>();
                        //dbi.CreateTable<HitomiIndexLanguagesColumnModel>();
                        //dbi.CreateTable<HitomiIndexTypesColumnModel>();
                        //dbi.CreateTable<HitomiIndexTagsColumnModel>();
                        //dbi.InsertAll(index.Item2.Select(md =>
                        //{
                        //    var dd = new HitomiColumnModel
                        //    {
                        //        Id = md.ID,
                        //        Artists = (md.Artists != null && md.Artists.Length > 0 ? "|" + string.Join("|", md.Artists) + "|" : "|N/A|"),
                        //        Characters = (md.Characters != null && md.Characters.Length > 0 ? "|" + string.Join("|", md.Characters) + "|" : null),
                        //        Groups = (md.Groups != null && md.Groups.Length > 0 ? "|" + string.Join("|", md.Groups) + "|" : null),
                        //        Series = (md.Parodies != null && md.Parodies.Length > 0 ? "|" + string.Join("|", md.Parodies) + "|" : null),
                        //        Title = md.Name,
                        //        Tags = (md.Tags != null && md.Tags.Length > 0? "|" + string.Join("|", md.Tags) + "|" : null),
                        //        Type = md.Type.ToString(),
                        //        Language = md.Language.ToString(),
                        //        Published = md.DateTime
                        //    };
...

                    {
                        ...
                        db.InsertAll(HitomiData.Instance.metadata_collection.Where(md => md.Language == null || md.Language == "" || md.Language == "korean").Select(md =>
                        {
                            ...
                            {
                                var ii = xxx[dict[md.ID.ToString()]];
                                dd.Uploader = ii.Uploader.Replace(" ", "");
                                dd.Published = DateTime.Parse(ii.Published);
                                dd.EHash = ii.URL.Split('/')[5];
                                dd.Files = ii.Files.Split(' ')[0].ToInt();
                                if (ii.Title.StartsWith("("))
                ...
                // Make Tag Index
                {
                    ...
                    {
                        insert(xx.tags, article.Tags);
                        insert(xx.artists, article.Artists);
                        insert(xx.groups, article.Groups);
                        insert(xx.series, article.Parodies);
                        insert(xx.characters, article.Characters);
                        insertSingle(xx.languages, article.Language);
                        insertSingle(xx.types, article.Type);
                        if (dict.ContainsKey(article.ID.ToString()))
                        {
                            var ii = xxx[dict[article.ID.ToString()]];
                            insertSingle(xx.uploaders, ii.Uploader);
                            if (ii.Title.StartsWith("("))
                            {
                                insertSingle(xx.classes, ii.Title.Split("(")[1].Split(")")[0]);
                            }
                        }
                    }

                    File.WriteAllText("index.json", JsonConvert.SerializeObject(xx));
                }
                ...
                // Make Artists Tags
                {
                    ...
                    foreach (var article in HitomiData.Instance.metadata_collection)
                    {
                        ...
                        {
                            foreach (var artist in article.Artists)
                            {
                        ...
                        {
                            foreach (var artist in article.Groups)
                            {
                    ...
                    File.WriteAllText("tag-index.json", JsonConvert.SerializeObject(ff));
                    File.WriteAllText("tag-artist.json", JsonConvert.SerializeObject(result_artist));
                    File.WriteAllText("tag-group.json", JsonConvert.SerializeObject(result_group));
                }
            }
